...
int main() {
    ...
    cout << "\n7. ВАЛИДАЦИЯ КЛЮЧА\n";
    cout << "───────────────────────────────────────────────────────────────\n";
    cout << (isValidKey(1, 'E') ? "✓" : "✗") << " isValidKey(1, 'E') = true\n";
    testsRun++; if (isValidKey(1, 'E')) testsPassed++;
    cout << (isValidKey(25, 'E') ? "✓" : "✗") << " isValidKey(25, 'E') = true\n";
    testsRun++; if (isValidKey(25, 'E')) testsPassed++;
    cout << (!isValidKey(0, 'E') ? "✓" : "✗") << " isValidKey(0, 'E') = false\n";
    testsRun++; if (!isValidKey(0, 'E')) testsPassed++;
    cout << (!isValidKey(26, 'E') ? "✓" : "✗") << " isValidKey(26, 'E') = false\n";
    testsRun++; if (!isValidKey(26, 'E')) testsPassed++;
    cout << (isValidKey(1, 'R') ? "✓" : "✗") << " isValidKey(1, 'R') = true\n";
    testsRun++; if (isValidKey(1, 'R')) testsPassed++;
    cout << (isValidKey(32, 'R') ? "✓" : "✗") << " isValidKey(32, 'R') = true\n";
    testsRun++; if (isValidKey(32, 'R')) testsPassed++;
    cout << (!isValidKey(33, 'R') ? "✓" : "✗") << " isValidKey(33, 'R') = false\n";
    ...
    if (testsRun > 0) {
        int percent = testsPassed * 100 / testsRun;
        cout << "║ Процент успеха: " << percent << "% " << string(28 - to_string(percent).length(), ' ') << "║\n";
    }
    cout << "╚════════════════════════════════════════════════════════════════╝\n\n";
