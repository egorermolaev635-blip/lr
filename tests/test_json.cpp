...
int main() {
    ...
    cout << (json1 == "\"test\"" ? "✓" : "✗") << " Сериализация строки\n";
    testsRun++;
    if (json1 == "\"test\"") testsPassed++;

    JsonValue val2(42.0);
    string json2 = jsonToString(val2, false);
    cout << (json2.find("42") != string::npos ? "✓" : "✗") << " Сериализация числа\n";
    testsRun++;
    if (json2.find("42") != string::npos) testsPassed++;
    ...
    testsRun++;
    if (json3.find("[") != string::npos && json3.find("]") != string::npos) testsPassed++;

    cout << "\n╔════════════════════════════════════════════════════════════════╗\n";
    cout << "║ РЕЗУЛЬТАТЫ ТЕСТОВ                                            ║\n";
    cout << "╠════════════════════════════════════════════════════════════════╣\n";
    cout << "║ Всего тестов: " << testsRun
         << " " << string(30 - to_string(testsRun).length(), ' ') << "║\n";
    cout << "║ Пройдено: " << testsPassed
         << " " << string(30 - to_string(testsPassed).length(), ' ') << "║\n";
    cout << "║ Не пройдено: " << (testsRun - testsPassed)
         << " " << string(30 - to_string(testsRun - testsPassed).length(), ' ') << "║\n";
    if (testsRun > 0) {
        int success = testsPassed * 100 / testsRun;
        cout << "║ Процент успеха: " << success
             << "% " << string(28 - to_string(success).length(), ' ') << "║\n";
    }
    cout << "╚════════════════════════════════════════════════════════════════╝\n\n";
